...
class HomeScreen:
    ...
    def switch_to_simulation_screen(self):
        ...
        SupplyChainSimulation(self.root)  # Call the callback to show the simulation screen
...
class SupplyChainSimulation:
    ...
    def create_simulation_screen(self):
        self.root.title("Healthcare aSupply Chain Simulation")
...
def load_hospitals():
    ...
    print(os.path.exists("hospitals.csv"))
    with open(r"C:\Users\HC\Documents\own\Holy_Hack\Team-4-Ctrl-Alt-Defeat-OMP\stijn\Hospitals.csv", 'r') as file:
...
def load_suppliers():
    suppliers = []
    with open(r"C:\Users\HC\Documents\own\Holy_Hack\Team-4-Ctrl-Alt-Defeat-OMP\stijn\Suppliers.csv", 'r') as file:
        reader = csv.DictReader(file)
        for row in reader:
            name = row['name']
            inventory = {
